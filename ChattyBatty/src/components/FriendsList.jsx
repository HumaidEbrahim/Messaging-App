...
const Friend = ({ friend }) => {
  return(
    ...
    <div><img className="w-10 rounded-full" src={friend.photo}/></div>
    <div>
      <div>{friend.username}</div>
      <div className="text-xs uppercase font-semibold opacity-60">{friend.status}</div>
    </div>
...
const FriendsList = ({ friendIds }) =>
{
  ...
  const fetchFriends = async () => {
    const friendPromises = friendIds.map(uid => getDoc(doc(db, 'users', uid)))
