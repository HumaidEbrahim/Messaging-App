        ...
        <div className="avatar online">
          <div className="w-24 rounded-full">
            <img src={user.photo}  />
              ...
              <p className="text-base-content/70"> Member Since: {dayjs(user?.creationDate?.toDate()).format('D MMMM YYYY')} </p>
              <p className="text-base-content/70" > {user.email} </p>
              <fieldset className="fieldset bg-base-200 border-base-300 rounded-box w-full border p-4">
  <legend className="fieldset-legend">Status</legend>
  <div className="join">
    <input type="text" className="input join-item" placeholder={status} onChange={e => setStatus(e.target.value)}/>
    <button className="btn join-item" onClick={updateStatus}><CgAddR size={20}/></button>
  </div>
</fieldset>
  <button className='btn btn-wide btn-primary btn-soft' onClick={logout}>LogOut</button>
         </div >
